...
typedef struct private_jitike_socket_t private_jitike_socket_t;

/**
 * Private data of an jitike_socket_t object.
 */
struct private_jitike_socket_t {

	/**
	 * Public jitike_socket_t interface.
	 */
	jitike_socket_t public;
...
METHOD(jitike_socket_t, push, void,
	private_jitike_socket_t *this, jitike_message_t *message)
{
	chunk_t chunk;
...
METHOD(jitike_socket_t, pull, jitike_message_t*,
	private_jitike_socket_t *this)
{
	while (TRUE)
	{
		...
		message = jitike_message_parse(chunk_create(buf, len));
		if (message)
		{
			return message;
		}
 ...
 */
static bool open_socket(private_jitike_socket_t *this)
{
...
METHOD(jitike_socket_t, destroy, void,
	private_jitike_socket_t *this)
{
	if (this->fd != -1)
	{
...
jitike_socket_t *jitike_socket_create(char *local, char *remote)
{
	...
	INIT(this,
		...
		.local = host_create_from_dns(local, 0, JITIKE_PORT),
		.remote = host_create_from_dns(remote, 0, JITIKE_PORT),
		.buflen = lib->settings->get_int(lib->settings,
										 "%s.plugins.jitike.buflen", 2048, lib->ns),
		.fd = -1,
	);
