...
typedef struct private_jitike_ike_t private_jitike_ike_t;

/**
 * Private data of an jitike_ike_t object.
 */
struct private_jitike_ike_t {

	/**
	 * Public jitike_ike_t interface.
	 */
	jitike_ike_t public;
...
METHOD(listener_t, ike_keys, bool,
	private_jitike_ike_t *this, ike_sa_t *ike_sa, diffie_hellman_t *dh,
	...
	shared_key_t *shared, auth_method_t method)
{
	...
	m = jitike_message_create(JITIKE_IKE_ADD);
	m->add_attribute(m, JITIKE_IKE_VERSION, ike_sa->get_version(ike_sa));
	m->add_attribute(m, JITIKE_IKE_ID, ike_sa->get_id(ike_sa));
	...
	if (rekey && rekey->get_version(rekey) == IKEV2)
	{
		...
		m->add_attribute(m, JITIKE_IKE_REKEY_ID, rekey->get_id(rekey));
		m->add_attribute(m, JITIKE_ALG_OLD_PRF, keymat->get_skd(keymat, &skd));
		m->add_attribute(m, JITIKE_OLD_SKD, skd);
	}

	proposal = ike_sa->get_proposal(ike_sa);
	if (proposal->get_algorithm(proposal, ENCRYPTION_ALGORITHM, &alg, &len))
	{
		m->add_attribute(m, JITIKE_ALG_ENCR, alg);
		if (len)
		{
			m->add_attribute(m, JITIKE_ALG_ENCR_LEN, len);
		}
	}
	if (proposal->get_algorithm(proposal, INTEGRITY_ALGORITHM, &alg, NULL))
	{
		m->add_attribute(m, JITIKE_ALG_INTEG, alg);
	}
	if (proposal->get_algorithm(proposal, PSEUDO_RANDOM_FUNCTION, &alg, NULL))
	{
		m->add_attribute(m, JITIKE_ALG_PRF, alg);
	}
	if (proposal->get_algorithm(proposal, DIFFIE_HELLMAN_GROUP, &alg, NULL))
	{
		m->add_attribute(m, JITIKE_ALG_DH, alg);
	}
	m->add_attribute(m, JITIKE_NONCE_I, nonce_i);
	m->add_attribute(m, JITIKE_NONCE_R, nonce_r);
	m->add_attribute(m, JITIKE_SECRET, secret);
	chunk_clear(&secret);
	if (ike_sa->get_version(ike_sa) == IKEV1)
	{
		if (dh->get_my_public_value(dh, &secret))
		{
			m->add_attribute(m, JITIKE_LOCAL_DH, secret);
			chunk_free(&secret);
		}
		m->add_attribute(m, JITIKE_REMOTE_DH, dh_other);
		if (shared)
		{
			m->add_attribute(m, JITIKE_PSK, shared->get_key(shared));
		}
		else
		{
			m->add_attribute(m, JITIKE_AUTH_METHOD, method);
		}
	}
	m->add_attribute(m, JITIKE_REMOTE_ADDR, ike_sa->get_other_host(ike_sa));
...
METHOD(listener_t, ike_updown, bool,
	private_jitike_ike_t *this, ike_sa_t *ike_sa, bool up)
{
	jitike_message_t *m;
	...
	if (up)
	{
		...
		m = jitike_message_create(JITIKE_IKE_UPDATE);
		m->add_attribute(m, JITIKE_IKE_ID, id);
		m->add_attribute(m, JITIKE_LOCAL_ID, ike_sa->get_my_id(ike_sa));
		m->add_attribute(m, JITIKE_REMOTE_ID, ike_sa->get_other_id(ike_sa));
		eap_id = ike_sa->get_other_eap_id(ike_sa);
		if (!eap_id->equals(eap_id, ike_sa->get_other_id(ike_sa)))
		{
			m->add_attribute(m, JITIKE_REMOTE_EAP_ID, eap_id);
		}
		m->add_attribute(m, JITIKE_LOCAL_ADDR, ike_sa->get_my_host(ike_sa));
		m->add_attribute(m, JITIKE_REMOTE_ADDR, ike_sa->get_other_host(ike_sa));
		m->add_attribute(m, JITIKE_CONDITIONS, condition);
		m->add_attribute(m, JITIKE_EXTENSIONS, extension);
		m->add_attribute(m, JITIKE_CONFIG_NAME, peer_cfg->get_name(peer_cfg));
		enumerator = ike_sa->create_peer_address_enumerator(ike_sa);
		while (enumerator->enumerate(enumerator, (void**)&addr))
		{
			m->add_attribute(m, JITIKE_PEER_ADDR, addr);
		}
		enumerator->destroy(enumerator);
	}
	else
	{
		m = jitike_message_create(JITIKE_IKE_DELETE);
		m->add_attribute(m, JITIKE_IKE_ID, ike_sa->get_id(ike_sa));
	}
	this->socket->push(this->socket, m);
	this->cache->cache(this->cache, ike_sa, m);
...
METHOD(listener_t, ike_rekey, bool,
	private_jitike_ike_t *this, ike_sa_t *old, ike_sa_t *new)
{
...
METHOD(listener_t, alert, bool,
	private_jitike_ike_t *this, ike_sa_t *ike_sa, alert_t alert, va_list args)
{
	switch (alert)
	{
...
METHOD(listener_t, ike_state_change, bool,
	private_jitike_ike_t *this, ike_sa_t *ike_sa, ike_sa_state_t new)
{
 ...
 */
static void sync_vips(private_jitike_ike_t *this, ike_sa_t *ike_sa)
{
	jitike_message_t *m = NULL;
	enumerator_t *enumerator;
	host_t *vip;
	...
	while (enumerator->enumerate(enumerator, &vip))
	{
		...
		m->add_attribute(m, JITIKE_REMOTE_VIP, vip);
	}
	enumerator->destroy(enumerator);
...
METHOD(listener_t, message_hook, bool,
	private_jitike_ike_t *this, ike_sa_t *ike_sa, message_t *message,
	bool incoming, bool plain)
{
	...
	if (plain && ike_sa->get_version(ike_sa) == IKEV2)
	{
		...
		{	/* we sync on requests, but skip it on IKE_SA_INIT */
			...
			if (incoming)
			{
				m = jitike_message_create(JITIKE_IKE_MID_RESPONDER);
			}
			else
			{
				m = jitike_message_create(JITIKE_IKE_MID_INITIATOR);
			}
			m->add_attribute(m, JITIKE_IKE_ID, ike_sa->get_id(ike_sa));
			m->add_attribute(m, JITIKE_MID, message->get_message_id(message) + 1);
			this->socket->push(this->socket, m);
			this->cache->cache(this->cache, ike_sa, m);
		}
	...
	if (ike_sa->get_version(ike_sa) == IKEV1)
	{
		...
		if (incoming == plain)
		{
			if (message->get_message_id(message) == 0)
			{
				...
				{
					m = jitike_message_create(JITIKE_IKE_IV);
					m->add_attribute(m, JITIKE_IKE_ID, ike_sa->get_id(ike_sa));
					m->add_attribute(m, JITIKE_IV, iv);
					this->socket->push(this->socket, m);
					this->cache->cache(this->cache, ike_sa, m);
				}
			}
		...
		message->get_exchange_type(message) == INFORMATIONAL_V1)
	{
		...
		if (notify)
		{
			...
			if (data.len == 4)
			{
				...
				m->add_attribute(m, JITIKE_IKE_ID, ike_sa->get_id(ike_sa));
				m->add_attribute(m, JITIKE_MID, seq + 1);
				this->socket->push(this->socket, m);
				this->cache->cache(this->cache, ike_sa, m);
			}
		}
...
METHOD(jitike_ike_t, destroy, void,
	private_jitike_ike_t *this)
{
	free(this);
}
