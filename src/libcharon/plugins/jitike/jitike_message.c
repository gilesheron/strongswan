...
typedef struct private_jitike_message_t private_jitike_message_t;

/**
 * Private data of an jitike_message_t object.
 */
struct private_jitike_message_t {

	/**
	 * Public jitike_message_t interface.
	 */
	jitike_message_t public;
...
ENUM(jitike_message_type_names, JITIKE_IKE_ADD, JITIKE_IKE_IV,
	"IKE_ADD",
	"IKE_UPDATE",
	"IKE_MID_INITIATOR",
	"IKE_MID_RESPONDER",
...
METHOD(jitike_message_t, get_type, jitike_message_type_t,
	private_jitike_message_t *this)
{
 ...
 */
static void check_buf(private_jitike_message_t *this, size_t len)
{
	int increased = 0;
...
METHOD(jitike_message_t, add_attribute, void,
	private_jitike_message_t *this, jitike_message_attribute_t attribute, ...)
{
	size_t len;
	va_list args;
...
{
	jitike_message_attribute_t attr, *attr_out;
...
METHOD(jitike_message_t, create_attribute_enumerator, enumerator_t*,
	private_jitike_message_t *this)
{
...
METHOD(jitike_message_t, get_encoding, chunk_t,
	private_jitike_message_t *this)
{
	return this->buf;
}

METHOD(jitike_message_t, destroy, void,
	private_jitike_message_t *this)
{
	free(this->buf.ptr);
	free(this);
...
static private_jitike_message_t *jitike_message_create_generic()
{
	private_jitike_message_t *this;
...
{
	private_jitike_message_t *this = jitike_message_create_generic();

	this->allocated = ALLOCATION_BLOCK;
	this->buf.ptr = malloc(this->allocated);
	this->buf.len = 2;
	this->buf.ptr[0] = JITIKE_MESSAGE_VERSION;
	this->buf.ptr[1] = type;
...
jitike_message_t *jitike_message_parse(chunk_t data)
{
	...
	if (data.ptr[0] != JITIKE_MESSAGE_VERSION)
	{
		DBG1(DBG_CFG, "HA message has version %d, expected %d",
			 data.ptr[0], JITIKE_MESSAGE_VERSION);
		return NULL;
	}

	this = jitike_message_create_generic();
