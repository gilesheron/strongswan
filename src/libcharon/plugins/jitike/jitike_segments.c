...
typedef struct private_jitike_segments_t private_jitike_segments_t;

/**
 * Private data of an jitike_segments_t object.
 */
struct private_jitike_segments_t {

	/**
	 * Public jitike_segments_t interface.
	 */
	jitike_segments_t public;
 ...
 */
static void log_segments(private_jitike_segments_t *this, bool activated,
						 u_int segment)
{
 ...
 */
static void enable_disable(private_jitike_segments_t *this, u_int segment,
						   bool enable, bool notify)
{
	ike_sa_t *ike_sa;
	enumerator_t *enumerator;
	ike_sa_state_t old, new;
	jitike_message_t *message = NULL;
	...
	if (notify)
	{
		message = jitike_message_create(type);
		message->add_attribute(message, JITIKE_SEGMENT, segment);
 ...
 */
static void enable_disable_all(private_jitike_segments_t *this, u_int segment,
							   bool enable, bool notify)
{
	int i;
...
METHOD(jitike_segments_t, activate, void,
	private_jitike_segments_t *this, u_int segment, bool notify)
{
	enable_disable_all(this, segment, TRUE, notify);
}

METHOD(jitike_segments_t, deactivate, void,
	private_jitike_segments_t *this, u_int segment, bool notify)
{
...
METHOD(listener_t, alert_hook, bool,
	private_jitike_segments_t *this, ike_sa_t *ike_sa, alert_t alert, va_list args)
{
 ...
 */
static job_requeue_t watchdog(private_jitike_segments_t *this)
{
 ...
 */
static void start_watchdog(private_jitike_segments_t *this)
{
...
METHOD(jitike_segments_t, handle_status, void,
	private_jitike_segments_t *this, segment_mask_t mask)
{
 ...
 */
static job_requeue_t send_status(private_jitike_segments_t *this)
{
	jitike_message_t *message;
	int i;

	message = jitike_message_create(JITIKE_STATUS);
	...
	for (i = 1; i <= this->count; i++)
	{
		if (this->active & SEGMENTS_BIT(i))
		{
			message->add_attribute(message, JITIKE_SEGMENT, i);
		}
	}
	this->mutex->unlock(this->mutex);
 ...
 */
static void start_heartbeat(private_jitike_segments_t *this)
{
	lib->processor->queue_job(lib->processor,
 ...
 */
static job_requeue_t autobalance(private_jitike_segments_t *this)
{
	int i, active = 0;
 ...
 */
static void start_autobalance(private_jitike_segments_t *this)
{
...
METHOD(jitike_segments_t, is_active, bool,
	private_jitike_segments_t *this, u_int segment)
{
	return (this->active & SEGMENTS_BIT(segment)) != 0;
}

METHOD(jitike_segments_t, count, u_int,
	private_jitike_segments_t *this)
{
	return this->count;
}

METHOD(jitike_segments_t, destroy, void,
	private_jitike_segments_t *this)
{
								  ...
								  bool monitor)
{
	...
	INIT(this,
		...
		.heartbeat_delay = lib->settings->get_int(lib->settings,
				"%s.plugins.jitike.heartbeat_delay", DEFAULT_HEARTBEAT_DELAY,
				lib->ns),
		.heartbeat_timeout = lib->settings->get_int(lib->settings,
				"%s.plugins.jitike.heartbeat_timeout", DEFAULT_HEARTBEAT_TIMEOUT,
				lib->ns),
		.autobalance = lib->settings->get_int(lib->settings,
				"%s.plugins.jitike.autobalance", 0, lib->ns),
	);
